...
FLR = data.frame(fuzzifikasi=0,left1=NA,left2=NA,right =NA)
for (i in 1:length(fuzifikasi)) {
  FLR[i,1] = fuzifikasi[i]
  FLR[i+2,2] = fuzifikasi[i]
  FLR[i+1,3] = fuzifikasi[i]
  FLR[i,4] = fuzifikasi[i]
}
FLR = FLR[-nrow(FLR),]
FLR = FLR[-1,]
FLR = FLR[-1,]
...
left=c("left1","left2")
FLR$left1left2=do.call(paste, c(FLR[left],sep=""))
FLR

library(dplyr)
FLR=FLR %>%
  mutate(left1left2=as.integer(left1left2)) %>%
  mutate(right=as.integer(right)) %>%
  select(fuzzifikasi,left1left2,right)
FLR

FLR=FLR %>%
  mutate(Rank = dense_rank(left1left2)) %>%
  select(fuzzifikasi,left1left2,right,Rank)
FLR
...
datapakai = data[c(3:length(data))]
galat = (datapakai-Prediksi[1:91,])
galat_kuadrat = galat^2
PE = abs(galat/datapakai*100)
tabel = cbind(datapakai,Prediksi[1:91,],galat,galat_kuadrat,PE)
tabel
